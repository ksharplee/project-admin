      ...
      <v-card>
        ...
        <v-card-text>
          <v-container fluid>
            <v-row>
              <v-col
                v-for="(item, i) in selectedProductsFromQrcode"
                :key="i"
                class="mb-5"
                cols="4"
                md="3"
                ...
                <div class="input-group">
                  ...
                  <div class="input-group-control">
                    <v-text-field
                      ...
                      hide-details
                      dense
                      @click:clear="clearSearchConditions('searchStr')"
                    />
                  </div>
                </div>
              </v-col>
...
export default {
  ...
  computed: {
    ...
    selectedProductsFromQrcode() {
      return R.filter(item => item.onlineErweimaPath, this.selectedProducts);
    },
    notNew() {
  ...
  methods: {
    ...
    getActiveCategory(arr) {
      ...
      this.$set(
        this.search,
        'categoryId',
        R.prop('id', R.head(this.categorySelected))
      );
      this.getProductList({
        p: '1',
      });
    ...
    getProductList(params) {
      ...
      this.getProductListAsync({ ...params, ...this.search })
        .catch((err) => {
          this.checkErr(err);
        })
        .finally(() => {
