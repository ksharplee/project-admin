          ...
          >
            <div class="input-group mb-5">
              ...
              <div class="input-group-control">
                <v-select
                  v-model="outstockOrder.warehouseId"
            ...
            <div class="input-group mb-5">
              <div class="input-group-prepend large">
                <span class="input-group-text required">选择出库类型</span>
              </div>
              <div class="input-group-control">
                <v-select
                  v-model="outstockOrder.outTypeId"
                  :items="warehouseOutstockTypeList.data.items"
                  :rules="outstockTypeRules"
                  placeholder="请选择出库类型"
                  dense
                  item-text="dnames"
                  item-value="id"
            ...
            <div class="input-group mb-5">
              <div class="input-group-prepend large">
                <span class="input-group-text">出库时间</span>
              </div>
              <div class="input-group-control">
                <v-menu
                  v-model="menuDate"
                ...
                >
                  <template v-slot:activator="{ on }">
                    <v-text-field
                      v-model="outstockOrder.deliveryTime"
                      class="white"
                      placeholder="请选择出库时间"
                      single-line
                      dense
            ...
            <div class="input-group mb-5">
              <div class="input-group-prepend large">
                <span class="input-group-text required">出库商品</span>
              </div>
              <div class="input-group-control">
                <v-btn
                  color="primary"
...
export default {
  ...
  data() {
    return {
      ...
      outstockTypeRules: [v => !!v || '请选择出库类型'],
      numberRules: [v => !!v || '请输入出库数量'],
      priceRules: [v => !!v || '请选择出库价格'],
      headers: [
        {
          text: '图片',
          value: 'image',
  ...
  computed: {
    ...mapState('warehouse', ['warehouseOutstockTypeList', 'warehouseList']),
  ...
  methods: {
    ...mapActions('warehouse', ['getWarehouseListAsync', 'getWarehouseOutstockTypeListAsync', 'addWarehouseOutstockAsync', 'getWarehouseOutstockDetailAsync', 'editWarehouseOutstockAsync']),
    ...
    loadProductListByWarehouseId() {
      // if (!this.outstockOrder.warehouseId) {
      //   this.checkErr(new Error('请先选择出库仓库'));
      //   return;
      // }
      ...
      this.getProductListForSelectAsync({ warehouseId: this.outstockOrder.warehouseId }).catch((err) => {
        ...
        this.dialogProductSelect = true;
      });
    },
    getAllData() {
      this.$store.commit('START_LOADING');
      const promises = [this.getWarehouseOutstockTypeListAsync(), this.getWarehouseListAsync(), this.getWarehouseOutstockDetailAsync({ id: this.id })];
      Promise.all(promises).then((res) => {
        this.outstockOrder = R.nth(2, res);
        this.selectedProducts = R.map(item => ({
          ...item,
          price: item.price,
        }), this.outstockOrder.detail);
      }).catch((err) => {
    ...
    editWarehouseOutstock() {
      if (!this.selectedProducts.length) {
        this.checkErr(new Error('请选择出库商品'));
        return;
      }
      this.submitting = true;
      this.editWarehouseOutstockAsync(R.mergeRight(this.outstockOrder, { detail: this.detail })).then(() => {
        ...
        this.checkErr(err, 'addWarehouseOutstock');
      }).finally(() => {
