<template>
  ...
  >
    ...
    <warehouse-outstock-type-single
      :show="dialogOutstockType"
      @close-dialog="dialogOutstockType = false"
    />
  </v-form>
</template>
...
export default {
  name: 'WarehouseOutstockAdd',
  components: { WarehouseSingle, WarehouseProductSelect, WarehouseOutstockTypeSingle },
  data() {
    return {
      loadingDataItems: false,
      dialogProductSelect: false,
      dialogWarehouse: false,
      dialogOutstockType: false,
      valid: true,
      menuDate: false,
      submitting: false,
      outstockOrder: {
  ...
  methods: {
    ...mapActions('warehouse', ['getWarehouseListAsync', 'getWarehouseOutstockTypeListAsync', 'addWarehouseOutstockAsync', 'editWarehouseOutstockAsync']),
    ...
    addWarehouseOutstock() {
      if (!this.selectedProducts.length) {
        this.checkErr(new Error('请选择出库商品'));
        return;
      }
      this.submitting = true;
      this.addWarehouseOutstockAsync({
        ...this.outstockOrder,
        detail: this.detail,
      }).then(() => {
